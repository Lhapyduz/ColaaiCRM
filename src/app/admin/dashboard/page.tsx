    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            try {
                ...
                activeStoresCount = activeStores || 0;
                ...
                totalMRR = activeSubscriptions.reduce((sum: number, s: SubscriptionRow) => sum + (s.amount_cents || 0), 0);

                // Generate demo monthly data for charts
                const months = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'];
                const currentMonth = new Date().getMonth();
                const numMonths = currentMonth + 1;

                for (let i = 0; i < numMonths; i++) {
                    demoMonthlyData.push({
                        month: months[i],
                        mrr: Math.floor(1000 + (i * 500) + Math.random() * 200),
