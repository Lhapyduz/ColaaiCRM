...
const PLANS = [
    { id: 'Basico' as PlanType, name: 'Básico', price: 'R$ 49,00', period: '/mês', priceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_BASIC, features: [{ name: 'Dashboard em Tempo Real', included: true }, { name: 'Gestão de Pedidos', included: true }, { name: 'Até 30 Produtos', included: true }, { name: 'Até 5 Categorias', included: true }, { name: 'Relatórios Básicos', included: true }, { name: 'Suporte por Email', included: true }, { name: 'Cardápio Online', included: false }, { name: 'Gestão de Cozinha', included: false }] },
    { id: 'Avançado' as PlanType, name: 'Avançado', price: 'R$ 79,00', period: '/mês', priceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_PROFESSIONAL, recommended: true, features: [{ name: 'Tudo do Básico +', included: true }, { name: 'Até 100 Produtos', included: true }, { name: 'Tela de Cozinha', included: true }, { name: 'Gestão de Entregas', included: true }, { name: 'Controle de Estoque', included: true }, { name: 'Programa de Fidelidade', included: true }, { name: 'Cardápio Online', included: true }, { name: 'Até 5 Funcionários', included: true }] },
    { id: 'Profissional' as PlanType, name: 'Profissional', price: 'R$ 149,00', period: '/mês', priceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_ENTERPRISE, features: [{ name: 'Tudo do Avançado +', included: true }, { name: 'Produtos Ilimitados', included: true }, { name: 'Cupons de Desconto', included: true }, { name: 'Previsão de Vendas (IA)', included: true }, { name: 'Funcionários Ilimitados', included: true }, { name: 'Relatórios Completos', included: true }, { name: 'Suporte Prioritário 24/7', included: true }] }
];
...
const AssinaturaPage = () => {
    ...
    const handlePixSubscribe = (planType: PlanPriceKey, planName: string) => { if (!user) { router.push('/login?redirect=/assinatura'); return; } setPixModalPlan({ type: planType, name: planName }); };
    const handlePixPaymentConfirmed = async () => { if (!pixModalPlan) return; setPixLoading(true); try { const response = await fetch('/api/pix/create-subscription', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ planType: pixModalPlan.type, amount: PLAN_PRICES[pixModalPlan.type] }) }); if (!response.ok) throw new Error(await response.text() || 'Falha ao processar pagamento Pix'); const data = await response.json(); alert(data.message); await refreshSubscription(); setPixModalPlan(null); } catch (error: any) { alert(`Erro: ${error.message}`); } finally { setPixLoading(false); } };
        ...
        <MainLayout>
            <div className="max-w-[1200px] mx-auto">
                ...
                <div className="flex flex-col items-center mb-8">
                    ...
                    {paymentMethod === 'pix' && <p className="text-xs text-text-muted mt-2">Pagamento via PIX direto. Assinatura ativada em até 24h após confirmação.</p>}
                </div>
                    ...
                    {PLANS.map((plan) => {
                        ...
                        return (
                            <div key={plan.id} className={cn('relative bg-bg-card border border-border rounded-lg p-6 flex flex-col transition-all duration-normal hover:border-primary/50', plan.recommended && 'border-primary ring-2 ring-primary/20')}>
                                ...
                                <div className="text-center my-4"><span className="text-3xl font-bold text-primary">{plan.price}</span><span className="text-text-muted">{plan.period}</span></div>
                                <div className="text-center text-xs text-[#2ecc71] font-medium mb-4">3 dias de teste grátis</div>
                                <div className="flex-1 flex flex-col gap-3 mb-6">{plan.features.map((feature, index) => (<div key={index} className={cn('flex items-center gap-2 text-sm', !feature.included && 'opacity-50')}>{feature.included ? <FiCheck className="text-[#27ae60] shrink-0" /> : <FiX className="text-[#e74c3c] shrink-0" />}<span className={!feature.included ? 'line-through' : ''}>{feature.name}</span></div>))}</div>
                                {isCurrentPlan ? <div className="py-3 px-4 bg-bg-tertiary rounded-md text-center text-text-secondary font-medium">Seu Plano Atual</div> : paymentMethod === 'pix' ? <Button variant={plan.recommended ? 'primary' : 'outline'} onClick={() => handlePixSubscribe(plan.id as PlanPriceKey, plan.name)} disabled={!!loadingCheckout}>{subscription ? 'Mudar Plano' : 'Pagar com PIX'}</Button> : <Button variant={plan.recommended ? 'primary' : 'outline'} onClick={() => handleSubscribe(plan.priceId, plan.id)} isLoading={loadingCheckout === plan.id} disabled={!!loadingCheckout}>{subscription ? 'Mudar Plano' : 'Assinar Agora'}</Button>}
                            </div>
                        );
                    })}
                </div>
            </div>
            {pixModalPlan && <PixPaymentModal planType={pixModalPlan.type} planName={pixModalPlan.name} onClose={() => setPixModalPlan(null)} onPaymentConfirmed={handlePixPaymentConfirmed} isLoading={pixLoading} />}
        </MainLayout>
    );
}
