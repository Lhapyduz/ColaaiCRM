...
export default function AbacatepayPixModal({
    ...
    // useEffect movido para baixo de createBilling para evitar TDZ

    const createBilling = useCallback(async () => {
        setLoading(true);
        setError(null);
    ...
    return (
        <Portal>
            <div
                className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-overlay p-4 animate-fadeIn"
                onClick={onClose}
            >
                <div
                    className="relative bg-bg-card rounded-2xl max-w-[420px] w-full max-h-[90vh] overflow-y-auto shadow-[0_20px_40px_rgba(0,0,0,0.2)] animate-scaleIn z-modal"
