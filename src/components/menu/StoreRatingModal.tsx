...
import { addStoreRating } from '@/actions/store';
...
export default function StoreRatingModal({ isOpen, onClose, storeOwnerId, appName }: StoreRatingModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            // Check local storage for recent rating
            const lastRated = localStorage.getItem(`store_rated_${storeOwnerId}`);
            if (lastRated) {
